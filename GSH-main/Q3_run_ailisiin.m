...
%figure;imagesc(lon, lat, moho); colorbar
while abs(e)>5e-5 && length(epsilon)<20
    tic;
    ...
    [data] = model_SH_synthesis(lonLim,latLim,height,SHbounds,V,Model);
    g_model=flip(data.vec.R);
    residual=g_obs-g_model;
%     figure;
%     imagesc(g_model); c=colorbar;


    moho=moho+residual.*alpha;
...
lon=0.5:1:359.5;
figure;
imagesc(lon, lat, flip(residual)); c=colorbar;
set(gca,'YDir','normal','Fontsize',11)
xlabel('Longitude (\circ)','Fontsize',22)
ylabel('Latitude (\circ)','Fontsize',22)
...
%%
figure;
imagesc(lon, lat, flip(residual).*1e5); c=colorbar;
set(gca,'YDir','normal','Fontsize',11)
xlabel('Longitude (\circ)','Fontsize',22)
ylabel('Latitude (\circ)','Fontsize',22)
ylabel(c,'Residual (mGal)','Fontsize',22)
...
%%
figure;
imagesc(lon, lat, moho./1e3); c=colorbar;
set(gca,'YDir','normal','Fontsize',11)
xlabel('Longitude (\circ)','Fontsize',22)
ylabel('Latitude (\circ)','Fontsize',22)
ylabel(c,'Thickness (km)','Fontsize',22); %sum lower density=-3.2745e9
%% save for later use
